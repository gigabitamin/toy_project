...
# -*- coding: utf-8 -*-
"""
HTML to Editable PPTX Converter V5
HTML 파일을 파싱하여 텍스트, 도형, 이미지를 편집 가능한 PPTX 객체로 변환하는 스크립트
- FontAwesome 아이콘을 실제 이미지로 다운로드하여 삽입
"""
...
class HTMLEditablePPTXConverterV5:
    ...
    def download_fontawesome_icon(self, icon_class, color='#2563eb', size=64):
        """FontAwesome 아이콘을 이미지로 다운로드"""
        try:
            # fas, fab, far 접두사 제거
            clean_class = icon_class.replace('fas ', '').replace('fab ', '').replace('far ', '')
            
            # FontAwesome CDN에서 SVG 다운로드
            svg_url = f"https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/svgs/solid/{clean_class.replace('fa-', '')}.svg"
            ...
            # SVG를 PNG로 변환하기 위해 임시 파일로 저장
            svg_path = self.temp_dir / f"{clean_class}.svg"
    ...
    def create_tech_stack_box(self, slide, tech_text, icon_path, x, y):
        """기술 스택 박스 생성"""
        try:
            # 배경 박스 생성
            ...
            if icon_path and icon_path.exists():
                try:
                    ...
                    slide.shapes.add_picture(str(icon_path), icon_left, icon_top, icon_width, icon_height)
                except Exception as e:
                    print(f"아이콘 이미지 추가 실패: {e}")
    ...
    def create_button(self, slide, text, icon_path, x, y, width, height, bg_color, text_color):
        """버튼 생성"""
        try:
            # 버튼 박스 생성
    ...
    def create_feature_card(self, slide, title, description, icon_path, x, y, width, height):
        """기능 카드 생성"""
        try:
            # 카드 배경 생성
    ...
    def create_icon_circle(self, slide, icon_path, x, y, size):
        """아이콘 원형 배경 생성"""
        try:
    ...
    def parse_01_html(self, soup, slide):
        ...
        if tech_label:
            ...
            if tech_container:
                ...
                for i, tech_item in enumerate(tech_items[:5]):
                    ...
                    if icon_elem:
                        ...
                        for cls in icon_classes:
                            if cls.startswith('fa-'):
                                icon_path = self.download_fontawesome_icon(cls, '#2563eb', 32)
                                break
                    ...
                    self.create_tech_stack_box(slide, tech_text, icon_path, x, y)
        ...
        if links_section:
            ...
            if len(buttons) > 0:
                ...
                if github_icon:
                    github_icon_path = self.download_fontawesome_icon('fa-github', '#ffffff', 32)
                self.create_button(
                    slide, github_text, github_icon_path,
                    4.5, 6.2, 2.5, 0.6,
            ...
            if len(buttons) > 1:
                ...
                deploy_icon_path = None
                if deploy_icon:
                    deploy_icon_path = self.download_fontawesome_icon('fa-globe', '#ffffff', 32)
                self.create_button(
                    slide, deploy_text, deploy_icon_path,
                    7.5, 6.2, 2.5, 0.6,
    ...
    def parse_02_html(self, soup, slide):
        ...
        if background_section:
            ...
            if parent_div:
                ...
                if icon_elem:
                    ...
                    for cls in icon_classes:
                        if cls.startswith('fa-'):
                            icon_path = self.download_fontawesome_icon(cls, '#2563eb', 48)
                            break
            
            # 아이콘 원형
            self.create_icon_circle(slide, icon_path, 0.5, 1.2, 0.6)
        ...
        if purpose_section:
            ...
            # 아이콘 원형
            self.create_icon_circle(slide, icon_path, 0.5, 2.9, 0.6)
        ...
        if features_section:
            ...
            # 아이콘 원형
            self.create_icon_circle(slide, icon_path, 0.5, 4.6, 0.6)
                ...
                for i, card in enumerate(feature_cards[:4]):
                    ...
                    if title_elem and desc_elem:
                        ...
                        if icon_elem:
                            ...
                            for cls in icon_classes:
                                if cls.startswith('fa-'):
                                    icon_path = self.download_fontawesome_icon(cls, '#2563eb', 24)
                                    break
                        
                        self.create_feature_card(slide, title, description, icon_path, x, y, card_width, card_height)
...
def main():
    ...
    output_path = r"C:\Project\gigabitamin\genspark\dcs_site\html\02_editable_v5.pptx"
    
    print("HTML to Editable PPTX 변환기 V5 시작")
    print(f"HTML 파일: {html_file}")
    print(f"출력 파일: {output_path}")
    print("-" * 50)
    
    # 변환기 생성 및 실행
    converter = HTMLEditablePPTXConverterV5(html_file, output_path)
